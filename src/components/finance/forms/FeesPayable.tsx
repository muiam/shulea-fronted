...
function FeesPayable() {
  ...
  const handleFeeRecipt = (
    ...
    navigate(`/receipt-fee/${year}/${term}/${grade}/${fee}`);
  };
  ...
  const handleFeeList = (
    ...
    navigate(`/received-fee/${year}/${term}/${grade}/${fee}`);
  };
  const viewFeeBalances = (year: number) => {
    navigate(`/fee-balances/${year}`);
  };
  const newFeePayable = () => {
    navigate(`/new/school/fee`);
  };
            ...
            <tbody>
              ...
              {fee.map((item, index) => (
                <tr key={index}>
                  ...
                  <td>
                    <button
                      onClick={() => viewFeeBalances(item.academic_year)}
                      style={{
                        width: "100%",
                        padding: "20px",
                        background: "aqua",
